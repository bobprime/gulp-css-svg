                  ...
                  encodeResource(result[1], file, opts, function (fileRes) {
                    if (undefined !== fileRes) {
                      ...
                      var strRes = 'data:' + mime.lookup(fileRes.path) + ';charset=utf8,';
										 new svgo().optimize(fileRes.contents.toString('utf8'), function(result) { strRes += result.data.replace(/"/g,"'").replace(/</g,'%3C').replace(/>/g,'%3E').replace(/{/g,'%7B').replace(/}/g,'%7D').replace(/#/g,'%23'); });
										 strRes = '"' + strRes + '"';
